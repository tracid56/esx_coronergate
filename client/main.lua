...
function lockcbv(success)
    ...
    TriggerEvent('esx:showNotification', 'Le déchiffrage des documents est fait!')
    TriggerServerEvent("esx_coronergate:ouvrir_journal")
    inprog = false
	else
		TriggerEvent('mhacking:hide')
    TriggerEvent('esx:showNotification', 'Le déchiffrage est faux!')
...
AddEventHandler('esx_coronergate:trycrack', function()
	if inprog == false then
	    ...
	    TriggerEvent("mhacking:start",7,30,lockcbv)
		 ...
		 ESX.ShowNotification('Déchiffrage en cours')
	end
end)
...
AddEventHandler('esx_coronergate:journal', function()
  Citizen.CreateThread(function()
    ...
    while display do
      Citizen.Wait(1)
      ShowInfo('~w~ Appuyez sur ~INPUT_CONTEXT~ pour fermer les archives', 0)
